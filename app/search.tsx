    ...
    useEffect(() => {
        const unsubscribe = subscribeToPosts((fetchedPosts) => {
            ...
            fetchedPosts.forEach(async (p) => {
                if (!users[p.userId]) {
                    const u = await getUserProfile(p.userId);
                    if (u) {
                        setUsers(prev => ({ ...prev, [p.userId]: u }));
                    }
                }
            });
        });
                        ...
                        {results.length === 0 ? (
                            ...
                            results.map(post => {
                                ...
                                const displayUser = poster ? {
                                    ...
                                    avatar: poster.avatarUrl || 'https://via.placeholder.com/150',
                                ...
                                return (
                                    <View key={post.id} style={{ marginBottom: 10 }}>
                                        <PostCard post={post} user={displayUser} />
                                    </View>
                                );
                            })
                        )}
